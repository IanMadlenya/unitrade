...
#ifndef InventoryManagment_UTIL_Header_h
#define InventoryManagment_UTIL_Header_h
...
public:
    ...
    void OpenIN(Para& para){
        ...
        if (!local_futures_book.is_open())
            ErrorMsg("cannot open local futures"+para.p_futuresconfig->futures_book);
    ...
    void OpenOUT(Para &para)
    {
        ...
        if (!futures_report_order.is_open())
            ErrorMsg("cannot open fut report order"+para.p_futuresconfig->FILE_TO_REPORT_ORDER);
        futures_order_detail.open(para.p_futuresconfig->FILE_TO_ORDER_DETAIL);
        if (!futures_order_detail.is_open())
            ErrorMsg("cannot open fut detail order"+para.p_futuresconfig->FILE_TO_ORDER_DETAIL);
        stk_report_order.open(para.p_stockconfig->FILE_TO_REPORT_ORDER);
        if (!stk_report_order.is_open())
            ErrorMsg("cannot open stk report order"+para.p_stockconfig->FILE_TO_REPORT_ORDER);
        stk_order_detail.open(para.p_stockconfig->FILE_TO_ORDER_DETAIL);
        if (!stk_order_detail.is_open())
            ErrorMsg("cannot open stk order detail"+para.p_stockconfig->FILE_TO_ORDER_DETAIL);
