...
inline static int readfn(void *handler, char *buf, int size) {
  fmem_t *mem =(fmem_t*) handler;
  size_t available = mem->size - mem->pos;
  
  if ((unsigned long)size > available) {
    size = available;
  }
...
inline static int writefn(void *handler, const char *buf, int size) {
  fmem_t *mem = (fmem_t*)handler;
  size_t available = mem->size - mem->pos;
...
inline static fpos_t seekfn(void *handler, fpos_t offset, int whence) {
  size_t pos;
  fmem_t *mem = (fmem_t*)handler;
...
inline static int closefn(void *handler) {
  free(handler);
  return 0;
}


inline FILE *fmemopen(void *buf, size_t size) {
