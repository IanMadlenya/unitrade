...
public:
    ...
    virtual void OnRtnFromBankToFutureByFutures(CThostFtdcRspTransferField *pRspTransfer) ;
    virtual void OnRtnFromFutureToBankByFutures(CThostFtdcRspTransferField *pRspTransfer) ;
	///请求查询资金账户
...
public:
	...
	int start(std::string name, std::string FRONT)
	{
		...
		if (handler==NULL)
		{
			std::cerr<<"--->>> CTPTrader"<<": cannot load libthosttraderapi.so"<<std::endl;
			return -1;
        	}
	       
		dlerror();
		CThostFtdcTraderApi * (*CreateTrader)(const char *);
		CreateTrader =(CThostFtdcTraderApi* (*)(const char*))  dlsym(handler,"_ZN19CThostFtdcTraderApi19CreateFtdcTraderApiEPKc");
		...
		TRADERAPI->SubscribePublicTopic(THOST_TERT_QUICK);
                ...
                std::cerr << "--->>> CTPtrader"<<": connected with server "  << std::endl;
